...
- 显示连接状态、房间码、双方准备状态。
- 保留现有 AI 对战入口。
- 复制邀请链接，支持 `?room=ABC123` 自动填入房间码。
- 双方准备时上传阵营与卡组 loadout。
- 服务器生成 match id 与 seed，并广播 `match_start`。

需要继续做：

- 增加更完整的失败重试提示。
- 在 UI 中更明确地展示双方已锁定阵营和卡组。
...
- 隐蔽信息不泄露。
- 胜负结算一致。
- 线上素材性能可控：卡牌插画、行动视频和音效按需加载或预加载，不阻塞出牌、目标选择、回合交接和 WebSocket 同步。
- 对线体验流畅：公网环境下部署、hover 预览、打击动画和结算特效无明显卡顿，控制台无资源加载失败或长任务堆积。
