...
CARD_DATA = ROOT / "repo_inspect" / "russia_cards_for_card_render.json"
ASSET_MANIFEST = ROOT / "repo_inspect" / "new_ru_card_assets_manifest.json"
MANIFEST_OUT = ROOT / "repo_inspect" / "generated_ru_card_manifest.json"
...
def split_source_name(stem):
    ...
    for suffix in DETAIL_SUFFIXES:
        if stem.endswith(suffix):
            return stem[: -len(suffix)], "detail", 1
...
def add_index_entry(index, base, slot, priority, path):
    for key in {base, normalize_key(base)}:
        entry = index.setdefault(key, {})
        previous_priority = entry.get(f"{slot}Priority", -1)
        if priority >= previous_priority:
            entry[slot] = path
            entry[f"{slot}Priority"] = priority
...
def load_asset_index():
    ...
    for entry in load_asset_manifest():
        ...
        path = Path(full_name)
        stem = Path(name).stem
        base, slot, priority = split_source_name(stem)
        if not slot:
            ignored.append(name)
            continue
        add_index_entry(index, base, slot, priority, path)
...
PANEL_OVERRIDES = {
    # The updated heavy-AA UI templates place their two skill frames lower than
    # the rest of the Russian deck, with taller first panels.
    "ru_buk_m3": [
        (248, 1035, 692, 160),
        (248, 1228, 692, 136),
    ],
...
def skill_panels(skill_count, card_id=None):
    if card_id in PANEL_OVERRIDES:
        return PANEL_OVERRIDES[card_id][:skill_count]
    if skill_count <= 1:
...
def render_detail(card, detail_path):
    ...
    for panel, (title, body) in zip(skill_panels(len(skills), card["id"]), skills):
...
def remove_old_ru_generated_images():
    ...
    removed = []
    for path in OUTPUT_ROOT.glob("ru_*.png"):
        path.unlink()
...
def main():
    cards = read_json(CARD_DATA)
    asset_index, ignored_assets = load_asset_index()
    removed = remove_old_ru_generated_images()
    generated = []
    missing = []
    ...
    manifest = {
        ...
        "generated": generated,
        "missing": missing,
        "missingDetailIds": [item["id"] for item in generated if item["art"] and not item["detail"]],
