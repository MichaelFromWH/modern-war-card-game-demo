...
来源：Michael 提供 `现代战争三线卡牌 V0.5.1 机制与全卡牌内容-20260523.docx`，确认按差异清单更新；F-35A 和 Su-57 保留 SEAD 专职定位，但当前不再使用独立的 `SEAD导弹` 标签或导弹裁决链。
...
- SEAD 机攻击防空单位时，按战斗机/重防拦截链处理，不再依赖 `SEAD导弹`。
